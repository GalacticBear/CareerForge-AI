...
const defaultAnalysis = { score: 0, semantic_score: 0, matched_skills: [], weak_skills: [], missing_skills: [], candidate_skills: [] };
...
export default function Dashboard({ user, onLogout }) {
  ...
  const load = async () => {
    ...
    if (profileRes.data.analyses?.[0]) {
      const a = profileRes.data.analyses[0];
      setAnalysis({ score: a.score, semantic_score: a.semanticScore, matched_skills: a.matchedSkills, weak_skills: a.weakSkills, missing_skills: a.missingSkills, candidate_skills: a.candidateSkills });
    }
  };
  ...
  const matchCoverage = analysis.required_skills?.length ? ((analysis.matched_skills.length + analysis.weak_skills.length) / analysis.required_skills.length) * 100 : analysis.score;
